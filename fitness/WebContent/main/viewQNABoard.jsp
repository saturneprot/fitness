...
/* 	$(function(){
	...
	function checkcheck(mid, b_sup, b_num){
		if(mid != b_sup){
			alert("본인 및 관계자 외 확인 불가");			
		}
		if(mid == b_sup){
			document.location='/fitness/main/viewRQuestion2.do?b_num='+b_num;
		}
	}
...
<body onload="onloading('${mid}','${cartT}')">
	...
	<div id="noticeR"><div id="noticeR-left">질문과답변</div><div id="noticeR-right"><input type="button" class="btn" value="글쓰기" onclick="writeQuestion('${mid}')"></div></div>
	<div id="floor3">	
		
		<table>
			<tr align="center">
				<td class="tdtd" width="70px">번호</td>
				<td class="tdtd" width="160px">카테고리</td>
				<td class="tdtd" width="600px">제목</td>
				<td class="tdtd" width="100px">작성자</td>
				<td class="tdtd" width="100px">작성일</td>
				<td class="tdtd" width="70px">조회수</td>
			</tr>
			...
			<c:if test="${list.size()>0 }">
				...
				<c:forEach var="bdto" items="${list}">
					<tr>
						...
						<c:if test="${bdto.bc_num==1}">
							<td align="center">질문과답변</td>
						</c:if>
						<c:if test="${bdto.mid eq 'admin'}">
						<%-- <td align="left"><span style="cursor:pointer;" onclick="document.location='${ctxpath}/main/beforeViewQuestion.do?b_num=${bdto.b_num}'"><img src="${ctxpath}/img/l-icon3.png" width="15px" height="15px">${bdto.b_subject}</span></td> --%>
						<td align="left"><span style="cursor:pointer;" onclick="checkcheck('${mid}','${bdto.b_sup}','${bdto.b_num}')"><img src="${ctxpath}/img/l-icon3.png" width="15px" height="15px">${bdto.b_subject}</span></td>
						</c:if>
						<c:if test="${bdto.mid ne 'admin'}">
						<td align="left"><span style="cursor:pointer;" onclick="document.location='${ctxpath}/main/beforeViewQuestion.do?b_num=${bdto.b_num}'">${bdto.b_subject}</span></td>
						</c:if>
				...
				<td align="center" colspan="6">
					...
					<c:if test="${totalRecord!=0}">
						...
						<%-- <c:if test="${currentBlock>0}"> --%>
							<a href="${ctxpath}/main/viewQNABoard.do?flag=userQ&block=0&page=0&keyField=${keyField}&keyWord=${keyWord}"><img src="${ctxpath}/img/doubleleft.png" width="10px" height="10px"></a>
						<%-- </c:if> --%>
						<%-- 이전블럭처리 --%>
						<c:if test="${currentBlock==0}">
						...
						<c:if test="${currentBlock>0}">
							<a href="${ctxpath}/main/viewQNABoard.do?flag=userQ&block=${currentBlock-1}&page=${(currentBlock-1)*pagePerBlock}&keyField=${keyField}&keyWord=${keyWord}"><img src="${ctxpath}/img/singleleft.png"></a>
						</c:if>
						...
						<c:forEach var="i" begin="1" end="${pagePerBlock}" step="1">
							<c:if test="${not loop_flag}">
								...
								<c:if test="${i!=(currentPage+1)}">
								<a href="${ctxpath}/main/viewQNABoard.do?flag=userQ&block=${currentBlock}&page=${currentBlock*pagePerBlock+i-1}&keyField=${keyField}&keyWord=${keyWord}">&nbsp;${i}</a>
								</c:if>
						...
						<c:if test="${totalBlock>(currentBlock+1)}">
							<a href="${ctxpath}/main/viewQNABoard.do?flag=userQ&block=${currentBlock+1}&page=${(currentBlock+1)*pagePerBlock}&keyField=${keyField}&keyWord=${keyWord}"><img src="${ctxpath}/img/singleright.png"></a>							
						</c:if>
						
						<%-- 맨마지막블럭의 마지막페이지로 가기 처리--%>
						<%-- <c:if test="${totalBlock}" --%>
						<a href="${ctxpath}/main/viewQNABoard.do?flag=userQ&block=${totalBlock-1}&page=${totalPage-1}&keyField=${keyField}&keyWord=${keyWord}"><img src="${ctxpath}/img/doubleright.png" width="10px" height="10px"></a>
		...
		<div id="searchF">
		<div id="searchSTH">
			<form name="searchThings" method="get" action="${ctxpath}/main/viewQNABoard.do">
				...
				<input type="hidden" name="flag" value="userQ">
