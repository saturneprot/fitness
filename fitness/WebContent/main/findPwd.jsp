	...
	<script>
		...
		function checkIt(){
			...
			if(document.pkc.mname.value!="" && document.pkc.mid.value!="" && document.pkc.memail.value!=""){
				$.ajax({
					type:"POST",
					url:"confirmNameIdEmail.jsp",
					data:"mname="+$("#mname").val()+"&mid="+$("#mid").val()+"&memail="+$("#memail").val(),
					dataType:'JSON',
					success:function(data){
						if(data.check==1){//유효한 아이디와 패스워드
							document.pkc.action="/fitness/main/findPwdPro.do";
							document.pkc.submit();
						}else if(data.check==-1){//불허받음
							alert("존재하지 않는 유저입니다.");	
						}//end else
					}//end success
				});//end ajax
...
<body>
	...
	<div id="logo" onclick="document.location='${ctxpath}/main/main.do'"><span id="logo0">운동</span>&nbsp;<span id="logo1">중</span><span id="logo2">독</span><span id="logo3">자</span><span id="logo4">들</span></div>
		<div id="contentR">
			...
			<div class="idR2">아이디</div>
			<div class="idRR"><input id="mid" class="input" onblur="checkcheck(this,1)" name="mid" type="text" placeholder="아이디를 입력해주세요"></div>
			<div class="message"></div>
			<div class="idR2">이메일</div>
			<div class="idRR"><input id="memail" class="input" onblur="checkcheck(this,2)" name="memail" type="text" placeholder="이메일을 입력해주세요"></div>
			<div class="message"></div>
			<div class="idR3">
