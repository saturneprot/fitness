...
<body onload="onloading('${mid}','${cartT}')">
	...
	<div id="noticeR">
		<div id="noticeR-left"><c:if test="${dto.bc_num eq 2}">공지사항</c:if><c:if test="${dto.bc_num eq 3}">FAQ</c:if></div>
		<div id="noticeR-right">
			<c:if test="${dto.bc_num eq 2}">
				<div class="noticeR-right1" onclick="document.location='${ctxpath}/main/viewNoticeBoard.do?flag=userN'">목록보기</div>
			</c:if>
			<c:if test="${dto.bc_num eq 3}">
				<div class="noticeR-right1" onclick="document.location='${ctxpath}/main/viewFAQBoard.do?flag=userF'">목록보기</div>
			</c:if>
		</div>
	</div>
	<div id="floor3">	
		<div id="contentRR">
			<div class="rowR">
				<div class="rowCC itemL1">게시글번호</div>
				<div class="rowCC itemR1">${dto.nb_num}</div>
				<div class="rowCC itemL1">카테고리</div>
				<div class="rowCC itemR1"><c:if test="${dto.bc_num eq 2}">공지사항</c:if><c:if test="${dto.bc_num eq 3}">FAQ</c:if></div>				
			</div>
			
			<div class="rowR">
				<div class="rowCC itemL1">게시자</div>
				<div class="rowCC itemR1"><c:if test="${dto.id eq 'admin'}">관리자</c:if></div>
				<div class="rowCC itemL1">조회수</div>
				<div class="rowCC itemR1">${dto.nb_count}</div>				
			</div>
			
			<div class="rowR">
				<div class="rowCC itemL1">글제목</div>
				<div class="rowCC itemR1">${dto.nb_subject}</div>
				<div class="rowCC itemL1">등록일</div>
				<div class="rowCC itemR1">${dto.nb_date}</div>				
			</div>
			<!-- <div class="rowR">
				<div class="rowCC itemL1">글내용</div>
			</div> -->
			<div style="margin-left:0px; width:1100px; height:675px; border:1px solid #eeeeee"><textarea style="width:100%; height:100%; border:none;" readonly>${dto.nb_content }</textarea></div>
		</div>
	</div>
