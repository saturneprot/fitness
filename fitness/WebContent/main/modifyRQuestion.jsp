...
<title>fitness</title>
	...
	<style>
		...
		table.tab thead th{border-bottom: 1px solid black; padding: 10px;}
		table.tab tbody td,th{border-bottom: 1px solid #ccc; padding: 10px;}
...
/* 	$(function(){
	...
	function checkContent(){
		if(!document.pkc.b_subject.value){
			alert("제목을 입력해주세요");
			return false;
		}
		if(!document.pkc.b_content.value){
			alert("내용을 입력해주세요");
			return false;
		}
		document.pkc.action="/fitness/main/modifyRQuestionPro.do";
		document.pkc.submit();
	}
...
<body onload="onloading('${mid}','${cartT}')">
	...
	<div id="floor3">	
		<form method="post" name="pkc">
		  ...
		  <table width="800" class="tab" cellpadding="0" cellspacing="0" align="center">
		   <thead>
		     <tr align="left">
		       <th colspan="2" style="font-size:24px;">질문과답변</th>
		      </tr>
		   </thead>
		   <tbody>     
		    <tr>
		      <td width="80" align="center" bgcolor="#f2f2f2">작성자</td>
		      <td width="720" >
		        <input type="text" class="text" name="writer" id="writer" value="${dto.mid}" size="15" readonly>
		    ...
		    <tr>
		      <td align="center" bgcolor="#f2f2f2">제목</td>
		      <td>
		        <input type="text" name="b_subject" id="subject" class="text" value="${dto.b_subject}">      
		      </td>
		    </tr>
		    
		    
		    <tr>
		      <td align="center" bgcolor="#f2f2f2">내용</td>
		      <td>
		        <textarea name="b_content" id="content" rows="20" cols="80">${dto.b_content}</textarea>
		      </td>
		    </tr>
		  ...
		  <br><br>
		  <div align="center">
		   <input type="button" class="btn" value="수정하기" onclick="checkContent()">
		   <input type="button" class="btn" value="나가기" onclick="javascript:history.back()">
		  </div>
		</form>
	</div>
